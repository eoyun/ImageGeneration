...
def Make_CondorScr(outname,dataset) :
    ...
    f.write(f'+JobBatchName = \"{outname}_{dataset}\"\n')
    f.write('Queue '+str(lines//10 + (1 if lines % 10 != 0 else 0))+'\n')
    f.close()
...
def Make_Scr(outname,dataset,script) :
    ...
    f.write('python3 '+script+' -d '+dataset+' -i $1 -o '+outname +' -l '+str(lines))
    f.close()
    return 0
...
if (os.path.isdir('./input/'+outname)) :
   print("plz check output name")
    ...
    for line in lines :
        ...
        Make_input(dataset,directory,outname,date) #add dR
        Make_Scr(outname,dataset,script)
